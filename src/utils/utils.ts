...
export const AddressToToken = Object.keys(PopularTokens).reduce((map, key) => {
...
export function getTokenName(
  ...
  return shorten ? mintAddress.substr(-3).toUpperCase() : mintAddress;
}
...
export function toLamports(
  ...
  const amount =
    typeof account === "number" ? account : Number.parseInt(account.info.amount?.toString());
...
export function fromLamports(
  ...
  typeof account === "number"
      ...
      : BN.isBN(account)
      ? account.toString()
      : account.info.amount.toString())
  try {
  amount = Math.floor(
    typeof account === "number"
      ? account
      : BN.isBN(account)
      ? Number.parseInt(account.toString())
      : Number.parseInt(account.info.amount.toString())
  );
  } catch {
    if (typeof account === "object" && !BN.isBN(account))
    notify({
      message: "Invalid amount",
      description: account.info.amount.toString(),
      type: "warn",
    });
    return 0;
  }
