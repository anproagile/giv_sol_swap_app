...
async function _addLiquidityExistingPool(
  ...
  const reserve0 = Number.parseInt(accountA.info.amount.toString());
  const reserve1 = Number.parseInt(accountB.info.amount.toString());
  const fromA =
  ...
  const supply = Number.parseInt(poolMint.supply.toString());
  const authority = poolMint.mintAuthority;
...
export async function calculateDependentAmount(
  ...
  const dependentTokenAmount = isFirstIndependent
    ? (Number.parseInt(accountB.info.amount.toString()) / Number.parseInt(accountA.info.amount.toString())) *
      adjAmount
    : (Number.parseInt(accountA.info.amount.toString()) / Number.parseInt(accountB.info.amount.toString())) *
      adjAmount;
