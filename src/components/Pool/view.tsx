...
import { AddressToToken, getPoolName, KnownTokenMap } from "../../utils/utils";
...
const PoolItem = (props: {
  ...
  const amount =
    Number.parseInt(item.account.info.amount.toString()) / Math.pow(10, mint?.decimals || 0);
  ...
  if (item) {
    return (
      ...
      >
        ...
        />
        <div className="pool-item-name">{getPoolName(AddressToToken.get(env) as KnownTokenMap, item.pool)}</div>
