...
import { useUserAccounts, useMint, useCachedPool } from "../../contexts/accounts";
import "./styles.less";
...
export const CurrencyInput = (props: {
  ...
  const grouppedUserAccounts = userAccounts
    .sort((a, b) => {
      // console.log(a.info.mint.toString())
      // console.log(b.info.mint.toString())
      // console.log(a.info.amount.toString())
      // console.log(b.info.amount.toString())
      return b.info.amount.cmp(a.info.amount);
    })
    .reduce((map, acc) => {
      // console.log("success");
      const mint = acc.info.mint.toBase58();
      if (isKnownMint(AddressToToken.get(env) as KnownTokenMap, mint)) {
        return map;
      }
  ...
  const renderAdditionalTokens = [...grouppedUserAccounts.keys()].map(
    (mint) => {
      ...
      if (account.pool) {
        name = getPoolName(AddressToToken.get(env) as KnownTokenMap, account.pool);
        ...
        name = getTokenName(AddressToToken.get(env) as KnownTokenMap, mint);
  ...
  const userUiBalance = () => {
    ...
    if (currentAccount && mint) {
      ...
      return (
        Number.parseInt(currentAccount.info.amount.toString()) / Math.pow(10, mint.decimals)
      );
    }
